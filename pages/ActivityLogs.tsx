...
const ActivityLogs: React.FC = () => {
  ...
  const getActionBadge = (action: ActionType) => {
    ...
    return (
      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${styles[action] || styles.SYSTEM}`}>
        {action}
      </span>
    );
  };
  ...
  const getEntityIcon = (entity: string) => {
    switch(entity) {
      ...
      case 'Attendance': return <Calendar size={14} />;
  ...
  const formatJakartaTime = (isoString: string) => {
      ...
      return {
          ...
          time: date.toLocaleTimeString('id-ID', { timeZone: 'Asia/Jakarta', hour: '2-digit', minute: '2-digit', second: '2-digit' })
      };
  };
      ...
      <div className="space-y-4">
            ...
            <table className="w-full text-left">
                <thead>
                    <tr className="bg-slate-850 border-b border-slate-800">
                        <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider w-48">Waktu (WIB)</th>
                        <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider w-32">User</th>
                        <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider w-24">Aksi</th>
                        <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider w-32">Modul</th>
                        <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Deskripsi Aktivitas</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-slate-800">
                    {currentData.map(log => {
                        const { date, time } = formatJakartaTime(log.timestamp);
                        return (
                        <tr key={log.id} className="hover:bg-slate-800 transition-colors group">
                            ...
                            <td className="px-6 py-4">
                                ...
                                <span className="text-[10px] text-slate-500 font-mono mt-1 block opacity-0 group-hover:opacity-100 transition-opacity">ID: {log.id}</span>
                            </td>
                        </tr>
                    )})}
          ...
          <div className="md:hidden space-y-3">
              {currentData.map(log => {
                  const { date, time } = formatJakartaTime(log.timestamp);
                  return (
                  <div key={log.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm relative overflow-hidden">
          ...
          {filteredLogs.length > 0 && (
            <div className="flex items-center justify-between pt-4">
                <div className="text-sm text-slate-500">
                    Menampilkan <span className="font-medium text-slate-200">{(currentPage - 1) * itemsPerPage + 1}</span> - <span className="font-medium text-slate-200">{Math.min(currentPage * itemsPerPage, filteredLogs.length)}</span> dari <span className="font-medium text-slate-200">{filteredLogs.length}</span>
                </div>
