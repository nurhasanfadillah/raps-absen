...
type CashAdvanceFormErrors = Partial<Record<keyof z.infer<typeof cashAdvanceSchema>, string>>;
...
const CashAdvancePage: React.FC = () => {
  ...
  const [formErrors, setFormErrors] = useState<CashAdvanceFormErrors>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const result = cashAdvanceSchema.safeParse({
      ...formData,
    ...
    if (!result.success) {
      ...
      result.error.issues.forEach((issue) => {
        const field = issue.path[0] as keyof CashAdvanceFormErrors;
    ...
    confirm({
      ...
      onConfirm: async () => {
        try {
          ...
          addToast('error', 'Gagal Menghapus', msg);
        }
      },
    });
  };
            ...
            >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm font-mono font-semibold text-slate-100 text-right mt-0.5">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    {!ca.isPaid ? (
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleMarkPaid(ca.id, emp?.fullName || '', ca.amount);
                        }}
                      >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDelete(ca.id, emp?.fullName || 'Kasbon');
                      }}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            {formErrors.employeeId && (
              <span className="text-xs text-red-400">{formErrors.employeeId}</span>
            )}
          </div>
          <Input
            label="Jumlah (IDR)"
          ...
          {editingId && (
            <div className="bg-amber-950/30 p-3 rounded-lg border border-amber-900/50 flex gap-3">
              ...
              <p className="text-xs text-amber-300">
                Mengedit nominal kasbon tidak akan mempengaruhi laporan gaji yang{' '}
                <strong>sudah</strong> dibuat sebelumnya. Pastikan laporan gaji periode terkait
                belum dibuat.
              </p>
            </div>
          )}
