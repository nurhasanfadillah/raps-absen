...
const Payroll: React.FC = () => {
  ...
  const handleGenerate = () => {
    ...
    if (result.items.length === 0) {
      ...
      addToast(
        'success',
        'Perhitungan Selesai',
        `Berhasil menghitung gaji untuk ${result.items.length} karyawan.`
      );
    }
    setReport(result);
  };
  ...
  const handleSave = () => {
    ...
    confirm({
      ...
      message:
        'Laporan ini akan disimpan sebagai FINAL. Status kasbon karyawan yang dipotong akan otomatis ditandai LUNAS. Lanjutkan?',
      variant: 'info',
  ...
  const handleDeleteHistory = (id: string, period: string) => {
    confirm({
      ...
      message: (
        <span>
          Menghapus laporan periode <strong>{period}</strong> akan{' '}
          <strong>membatalkan status pembayaran Kasbon</strong> yang terkait (rollback). Laporan
          akan dipindahkan ke arsip sampah (Soft Delete).
        </span>
      ),
      variant: 'danger',
      ...
      onConfirm: async () => {
        try {
          ...
          addToast(
            'success',
            'Laporan Dihapus',
            'Laporan berhasil dihapus dan status kasbon telah dikembalikan.'
          );
        } catch (e) {
          ...
          <div className="lg:col-span-1">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm sticky top-24">
              ...
              <p className="text-sm text-slate-500 mb-6">
                Pilih periode tanggal untuk menghitung gaji (Sistem Gaji Harian), lembur, dan
                potongan.
              </p>
              ...
              <div className="space-y-6 animate-fade-in">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-brand-950/20 border border-brand-900/50 p-4 rounded-xl">
                  <div>
                    ...
                    <p className="text-sm text-brand-400/70">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <Button
                      onClick={handleSave}
                      isLoading={isSaving}
                      className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 shadow-emerald-900/40"
                    >
                      <Save size={16} /> <span className="sm:hidden">Finalisasi</span>
                      <span className="hidden sm:inline">Simpan & Finalisasi</span>
                    </Button>
                  </div>
                </div>
        ...
        <div className="space-y-8 animate-fade-in">
          {visibleReports.length === 0 ? (
            <div className="py-12 text-center text-slate-500">
              Belum ada riwayat penggajian yang aktif.
            </div>
          ) : (
            visibleReports
              .slice()
              ...
              .map((rep) => (
                <Card
                  key={rep.id}
                  title={`Laporan: ${rep.periodStart} s/d ${rep.periodEnd}`}
                ...
                >
                  <div className="mb-4 flex flex-col md:flex-row justify-between md:items-center px-4 md:px-6 pt-2 gap-4">
                    ...
                    <div className="flex gap-2">
                      <Button
                        onClick={() =>
                          handleDeleteHistory(rep.id, `${rep.periodStart} - ${rep.periodEnd}`)
                        }
                        variant="danger"
                        className="h-9 md:h-8 text-xs px-3 md:px-2 flex-1 md:flex-none"
                      >
                        <Trash2 size={14} /> Hapus
                      </Button>
                      <Button
                        onClick={() => generateReportPDF(rep)}
                        variant="secondary"
                        className="h-9 md:h-8 text-xs flex-1 md:flex-none"
                      >
                        <Printer size={14} /> <span className="md:hidden">Cetak</span>
  ...
  <>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-850 border-b border-slate-800">
              ...
              <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase text-center">
                Efektif
              </th>
              <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase text-right">
                Gaji Dasar
              </th>
              <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase text-right">
                Lembur
          ...
          <tbody className="divide-y divide-slate-800">
            {report.items.map((item) => (
              <tr key={item.employeeId} className="hover:bg-slate-800 transition-colors">
                ...
                <td className="px-5 py-3 text-right font-mono font-bold text-slate-100 text-sm">
          ...
          <tfoot className="bg-slate-850/80 border-t border-slate-800 backdrop-blur-sm">
            <tr>
              <td
                colSpan={5}
                className="px-5 py-4 text-right text-sm font-bold text-slate-400 uppercase tracking-wide"
              >
                Total Pembayaran
              </td>
              <td className="px-5 py-4 text-right font-bold font-mono text-lg text-slate-100">
                {formatMoney(report.totalPayout)}
              </td>
              {!isPreview && <td></td>}
            </tr>
          </tfoot>
    ...
    <div className="md:hidden space-y-4">
      {report.items.map((item) => (
        <div
          key={item.employeeId}
          className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm relative"
        >
          ...
          <div className="grid grid-cols-2 gap-3 text-xs mb-4">
            <div className="bg-slate-800 p-2 rounded border border-slate-700">
              ...
              <div className="text-[10px] text-slate-500 mt-0.5">
                {item.totalEffectiveDays} Hari Efektif
              </div>
            </div>
            <div className="bg-slate-800 p-2 rounded border border-slate-700">
              <span className="text-slate-500 block mb-1">Lembur</span>
              <span className="font-mono text-emerald-400">
                {formatMoney(item.totalOvertimePay)}
              </span>
            </div>
            <div className="bg-slate-800 p-2 rounded border border-slate-700">
      ...
      <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 flex justify-between items-center sticky bottom-0 z-10 shadow-xl">
        ...
        <span className="font-bold font-mono text-lg text-slate-100">
          {formatMoney(report.totalPayout)}
        </span>
      </div>
    </div>
