...
export const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', className = '', isLoading, ...props }) => {
  const base = "px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]";
  
  const variants = {
    primary: "bg-gradient-to-r from-brand-600 to-brand-800 hover:from-brand-500 hover:to-brand-700 text-white shadow-lg shadow-brand-900/40 border border-transparent",
    ...
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      <div className="bg-slate-900 border border-slate-800 w-full md:w-full max-w-lg rounded-t-2xl md:rounded-xl shadow-2xl relative animate-slide-up md:animate-in md:fade-in md:zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900 shrink-0 rounded-t-2xl md:rounded-t-xl">
          ...
          <button onClick={onClose} className="text-slate-500 hover:text-slate-200 transition-colors p-1 hover:bg-slate-800 rounded-md">
            <X size={18} />
          </button>
        </div>
...
export const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
    ...
    if (status === 'Active') { color = 'bg-emerald-950/40 text-emerald-400 border-emerald-900/50'; label = 'Aktif'; }
    if (status === 'Present' || status === 'Paid') { color = 'bg-emerald-950/40 text-emerald-400 border-emerald-900/50'; label = status === 'Paid' ? 'Lunas' : 'Hadir'; }
    if (status === 'Inactive') { color = 'bg-rose-950/40 text-rose-400 border-rose-900/50'; label = 'Tidak Aktif'; }
    if (status === 'Alpha') { color = 'bg-rose-950/40 text-rose-400 border-rose-900/50'; label = 'Alpa'; }
    if (status === 'Sick') { color = 'bg-amber-950/40 text-amber-400 border-amber-900/50'; label = 'Sakit'; }
    if (status === 'Permission') { color = 'bg-blue-950/40 text-blue-400 border-blue-900/50'; label = 'Izin'; }
    
    return (
        <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold border ${color} uppercase tracking-wider shadow-sm`}>
            {label}
        </span>
    );
};