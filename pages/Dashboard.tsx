  ...
  <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 md:p-5 shadow-sm relative overflow-hidden group h-full flex flex-col justify-between hover:border-brand-900/50 transition-colors">
    ...
    <div className="relative z-10 flex flex-col h-full justify-between gap-2 md:gap-4">
      <div className="flex justify-between items-start">
        ...
        {trend && (
          <div
            className={`flex items-center gap-0.5 text-[10px] md:text-xs font-medium ${trendColor} bg-slate-800 px-1.5 py-0.5 md:px-2.5 md:py-1 rounded-full border border-slate-700 shadow-sm`}
          >
      ...
      <div>
        <h3 className="text-lg md:text-2xl font-bold text-slate-100 tracking-tight font-sans truncate">
          {value}
        </h3>
        <p className="text-[10px] md:text-sm text-slate-500 font-medium leading-tight mt-0.5 md:mt-1 truncate">
          {label}
        </p>
      </div>
    </div>
...
const Dashboard: React.FC = () => {
  ...
  const totalKasbon = cashAdvances
    .filter((c) => !c.isPaid)
  ...
  return (
    <div className="space-y-4 md:space-y-8">
      <div>
        ...
        <p className="text-xs md:text-base text-slate-500 mt-0.5 md:mt-1">
          Ringkasan aktivitas operasional hari ini (PT. Redone BMU).
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-6">
        ...
        />
        <StatCard icon={DollarSign} label="Kasbon Pending" value={formatMoney(totalKasbon)} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <Card className="lg:col-span-2 min-h-[280px] md:min-h-[400px]" title="Statistik Kehadiran">
          <div
            className="h-48 md:h-80 w-full mt-2 md:mt-4 -ml-4 md:ml-0"
            style={{ minWidth: 0, minHeight: '12rem' }}
          >
            ...
            <div className="space-y-0 relative max-h-[300px] md:max-h-[360px] overflow-y-auto pr-2 custom-scrollbar">
              ...
              {auditLogs.slice(0, 8).map((log) => {
                ...
                return (
                  <div
                    key={log.id}
                    className="flex gap-3 md:gap-4 items-start py-3 group relative border-b border-dashed border-slate-800 last:border-0"
                  >
                    <div
                      className={`w-8 h-8 rounded-full ${bg} border flex items-center justify-center ${color} text-xs shadow-sm z-10 shrink-0 mt-0.5`}
                    >
                      <Icon size={14} />
                    </div>
                    <div className="pt-0.5 flex-1 min-w-0">
                      <p className="text-xs md:text-sm text-slate-300 font-medium leading-snug break-words">
                        {log.description}
                      </p>
                      <span className="text-[10px] text-slate-500 block mt-1">
                        <span className="bg-slate-800 px-1.5 py-0.5 rounded text-slate-400 border border-slate-700">
                          {formatJakartaTime(log.timestamp)}
                        </span>
                      </span>
                    </div>
                  </div>
