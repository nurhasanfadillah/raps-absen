...
import { Card, Button, Input, Select, StatusBadge, Modal } from '../components/UIComponents';
import { ChevronLeft, ChevronRight, Save, Calendar, Edit3, Clock, AlertTriangle } from 'lucide-react';
...
const Attendance: React.FC = () => {
  ...
  const handleSaveModal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast('error', 'Gagal Menyimpan', 'Terjadi kesalahan saat menyimpan absensi.');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="w-full md:w-auto flex items-center justify-between md:justify-end gap-2 bg-slate-900 p-1.5 rounded-lg border border-slate-800 shadow-sm">
          <button onClick={() => changeDate(-1)} className="p-2 hover:bg-slate-800 rounded-md text-slate-500 hover:text-slate-200 transition-colors">
            <ChevronLeft size={18} />
          </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-850 border-b border-slate-800">
                ...
                <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-center">Waktu Masuk</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-center">Jam Lembur</th>
                        ...
                        {status === 'Present' && record?.checkInTime ? (
                            <div className="flex flex-col items-center">
                                ...
                                {timeStatus && timeStatus.color !== 'text-emerald-400' && (
                                    <span className={`text-[10px] ${timeStatus.color}`}>{timeStatus.label}</span>
                                )}
                            </div>
                        ) : '-'}
                    </td>
                    <td className="px-6 py-4 text-center text-sm font-mono text-slate-400">
                      {status === 'Present' && record?.overtimeHours ? `${record.overtimeHours} jam` : '-'}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <Button variant="secondary" className="ml-auto h-8 text-xs px-3" onClick={() => openEditModal(emp.id)}>
      ...
      <div className="md:hidden space-y-3">
         {activeEmployees.map(emp => {
            ...
            return (
              <div key={emp.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm flex flex-col gap-3">
                 ...
                 {status === 'Present' && (
                     <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="bg-slate-800 p-2 rounded border border-slate-700 flex flex-col items-center justify-center">
                            ...
                            <span className="font-mono font-medium text-sm text-slate-300">{record?.checkInTime || '-'}</span>
                            {timeStatus && timeStatus.color !== 'text-emerald-400' && (
                                <span className={`text-[10px] ${timeStatus.color} mt-0.5`}>{timeStatus.label}</span>
                            )}
                        </div>
                        <div className="bg-slate-800 p-2 rounded border border-slate-700 flex flex-col items-center justify-center">
                            <span className="text-slate-500 mb-1">Lembur</span>
                             <div className="flex items-center gap-1">
                                <Clock size={12} className={record?.overtimeHours ? 'text-amber-400' : 'text-slate-500'} />
                                <span className="font-mono font-medium text-sm text-slate-300">{record?.overtimeHours || 0}</span>
                             </div>
                        </div>
                     </div>
                 )}
                 ...
                 <Button variant="secondary" className="w-full text-xs h-9" onClick={() => openEditModal(emp.id)}>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Update Absensi & Lembur">
        <form onSubmit={handleSaveModal} className="space-y-5">
            <Select 
              label="Status Kehadiran" 
              value={modalForm.status} 
              onChange={e => setModalForm({...modalForm, status: e.target.value as AttendanceStatus})}
            >
                ...
                {modalForm.checkInTime > HALF_DAY_THRESHOLD_TIME ? (
                    <div className="bg-rose-950/30 p-3 rounded-lg border border-rose-900/50 flex gap-2 items-start">
                        ...
                        <div className="text-xs text-rose-300">
                           <strong>Peringatan Potongan:</strong> Jam masuk lewat dari {HALF_DAY_THRESHOLD_TIME}. Sistem akan menghitung kehadiran ini sebagai <strong>0.5 Hari</strong>.
                        </div>
                    </div>
                     ...
                     <div className="bg-amber-950/30 p-3 rounded-lg border border-amber-900/50 flex gap-2 items-start">
                        ...
                        <div className="text-xs text-amber-300">
                           <strong>Terlambat:</strong> Jam masuk lewat dari {WORK_START_TIME}. Tetap dihitung 1 Hari (Full) namun tercatat terlambat.
                        </div>
                    </div>
                ) : null}
            ...
            <div className="pt-6 flex justify-end gap-3 border-t border-slate-800">
              ...
              <Button type="submit"><Save size={16}/> Simpan Data</Button>
            </div>
        </form>
      </Modal>
    </div>
