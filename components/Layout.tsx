...
const NavItem = ({ to, icon: Icon, label, onClick }: any) => {
  return (
    ...
    >
      <Icon
        size={18}
        className={`transition-colors duration-200 ${({ isActive }: any) => (isActive ? 'text-brand-500' : 'text-slate-500 group-hover:text-slate-300')}`}
      />
      <span>{label}</span>
    </NavLink>
  );
    ...
    <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="text-xs font-bold text-slate-600 uppercase tracking-wider mt-8 mb-4 px-2">
            Keuangan
          </div>
          <NavItem
            to="/cash-advance"
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-slate-950">
        ...
        <header className="h-16 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md flex items-center justify-between px-4 lg:px-8 z-30 sticky top-0 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <div
              className={`hidden md:flex items-center gap-2 text-xs font-medium px-2.5 py-1 rounded-full border transition-colors duration-300 ${
                isOnline
        ...
        <div className="flex-1 overflow-auto p-4 lg:p-8 custom-scrollbar">
          <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-10">{children}</div>
        </div>
      </main>
    </div>
  );
